 ...
 *
 * Two entry points mount this: server/index.ts runs it as a long-lived Node
 * process (dev, or a VM), and api/[...path].ts hands it to Vercel as a
 * serverless function. Keeping the routes here means the two can never drift.
 */
...
import { allLeads, availability, backend, claim, type Lead } from './store.js';
...
app.post('/api/leads', async (req, res) => {
  ...
  if (typeof body.company === 'string' && body.company.trim() !== '') {
    return res.json({ ok: true, status: 'created', availability: await availability() });
  }
...
/**
 * Last-resort durability.
 *
 * On a host with no persistent disk and no Redis configured, the store is
 * effectively write-only — so every accepted lead is also emitted as a single
 * structured log line. Log retention is not a database, but it means a lead
 * is never silently gone: grep the platform logs for HOY_LEAD and you can
 * reconstruct them.
 *
 * Configure LEAD_WEBHOOK_URL, or Redis, and this becomes belt-and-braces.
 */
function record(lead: Lead): void {
  console.log(`HOY_LEAD ${JSON.stringify(lead)}`);
  if (backend === 'filesystem' && !LEAD_WEBHOOK_URL && process.env.VERCEL) {
    console.warn(
      '[store] Running on Vercel with no Redis and no webhook — this lead exists ' +
        'only in these logs. Set KV_REST_API_URL/KV_REST_API_TOKEN before sending traffic.',
    );
  }
}
...
app.get('/api/admin/leads.csv', async (req, res) => {
  ...
  const rows = (await allLeads()).map((l) =>
    cols
      .map((c) =>
...
// `store` tells you at a glance whether a serverless deploy picked up Redis
// or silently fell back to an ephemeral disk.
app.get('/api/health', (_req, res) => res.json({ ok: true, store: backend }));
